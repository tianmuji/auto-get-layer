...
interface TestResult {
  ...
  summary: {
    totalNodes: number;
    autoLayoutCapableNodes: number;
    autoLayoutEnabledNodes: number;
 ...
 * 生成测试摘要
 */
export function generateTestSummary(statistics: any, violations: any[]): any {
 ...
 * 生成 Markdown 格式的测试报告
 */
export function generateMarkdownReport(testResult: TestResult): string {
...
- **问题描述**: 
`;
      violation.violations.forEach((v: string) => {
        report += `  - ${v}\n`;
      });
      
      report += `- **修改建议**: 
`;
      violation.suggestions.forEach((s: string) => {
        report += `  - ${s}\n`;
      });
 ...
 * 生成 JSON 格式的测试报告
 */
export function generateJsonReport(testResult: TestResult): string {
 ...
 * 保存测试报告到文件
 */
export function saveTestReport(testResult: TestResult, outputDir: string = 'test/reports'): void {
