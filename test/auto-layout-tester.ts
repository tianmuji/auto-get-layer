 ...
 * 主测试函数
 */
async function runAutoLayoutTest(figmaUrl: string): Promise<void> {
  ...
  try {
    ...
    if (nodeId) {
      // 获取特定节点
      console.log('\n🔍 获取指定节点数据...');
      const nodeResponse = await getFigmaNode(fileKey, nodeId);
      ...
      // 同时获取文件信息以获取文件名
      const fileResponse = await getFigmaFile(fileKey);
      figmaData = fileResponse;
    } else {
      // 获取整个文件
      console.log('\n📄 获取文件数据...');
      figmaData = await getFigmaFile(fileKey);
